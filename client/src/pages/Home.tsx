...
export default function Home() {
  ...
  return <div className={isAr ? "site rtl" : "site ltr"} dir={isAr ? "rtl" : "ltr"}>
    ...
    <main>
      ...
      <section className="collection-section" id="collection"><div className="section-heading"><div><span className="section-kicker">THE COLLECTION</span><h2>{copy.collection}</h2></div></div><div className="watch-grid">{watches.map((watch, index) => <article className="watch-card" key={watch.id}><div className="watch-image-wrap"><img src={watch.image} alt={isAr ? "صورة ساعة ذكية" : "Smartwatch image"} loading={index > 2 ? "lazy" : "eager"}/></div><ul className="watch-features">{featureCatalog.map((feature) => { const Icon = feature.icon; return <li key={feature.ar}><Icon size={18} strokeWidth={2.2}/><span>{isAr ? feature.ar : feature.en}</span></li>; })}</ul><Link href={`/order?watch=${watch.id}`} className="card-button">{isAr ? "اطلب ساعات الآن" : "Order watches now"}</Link></article>)}</div></section>
